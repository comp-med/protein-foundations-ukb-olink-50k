...
## --> import protein levels fully adjusted <-- ##
ukb.prot <- fread("input/UKB.olink.proteins.full.20250516.txt")
...
if(sum(ukb.dat[, paste0("bin_", phe), with=F]) >= 200){
  ...
  ## store the results
  fwrite(res.surv, paste("output/res.surv.adj", phe, "txt", sep="."), sep="\t", row.names = F, na = NA)
