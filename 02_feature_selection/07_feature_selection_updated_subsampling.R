...
## correct directory
setwd("<path>/51_protein_variance_updated/02_feature_selection/")
...
pop       <- args[2]
## sample subgroup
pop.grp   <- as.numeric(args[3])

# ## for testing purpose: slmap - AFR - 4
# var.olink <- "slmap"
# pop       <- "AFR"
# pop.grp   <- 4
...
ukb.phe                       <- fread("../01_phenotype_prep/data/UKB.variables.variance.decomp.imputed.for.revision.20250624.txt")
...
ukb.phe                       <- ukb.phe[, c("f.eid", lab.phe$short_name, "controls.AFR", "controls.CSA"), with=F]
...
var.phe                       <- var.phe[ -which(var.phe %in% c("controls.AFR", "controls.CSA"))]
...
## create
ukb.phe                       <- data.table(ukb.phe[, c("f.eid", "controls.AFR", "controls.CSA")], tmp[,-1])
...
ukb.phe <- ukb.phe[ eval(as.name(paste0("controls.", pop))) == pop.grp]
...
write.table(id.feat, paste("tmpdir/feature.set", pop, pop.grp, var.olink, "txt", sep="."), sep = "\t", row.names=F)
...
## selection matrix
write.table(res.stab$phat, paste("output_updated/variable.matrix", pop, pop.grp, var.olink, "txt", sep="."), sep = "\t", row.names=T)
...
## imputed data set of phenotypes
ukb.var                       <- fread("../01_phenotype_prep/data/UKB.variables.variance.decomp.imputed.for.revision.20250624.txt", select=c("f.eid", unique(c(var.sel))))
...
## Explained variance
write.table(var.lasso, paste("output_updated/lasso.explained.variance", pop, pop.grp, var.olink, "txt", sep="."), sep = "\t", row.names=F)
