...
ukb.prot <- fread("input/UKB.olink.proteins.trans.genetic_scores.20250612.txt")

ukb.prot <- ukb.prot %>% 
  select(c(IID, starts_with("trans.score")))

## rename to fit to protein names
names(ukb.prot) <- sub("^trans.score.", "", names(ukb.prot))
...
if(sum(ukb.dat[, paste0("bin_", phe), with=F]) >= 200){
  ...
  ## store the results
  fwrite(res.surv, paste("output/res.surv.trans", phe, "txt", sep="."), sep="\t", row.names = F, na = NA)
