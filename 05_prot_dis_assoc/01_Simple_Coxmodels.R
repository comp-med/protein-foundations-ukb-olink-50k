...
## import phecode, associated cohort (sex-specific?) and type of proteins to be tested across proteins
phe    <- args[1]
cohort <- args[2]
# phe    <- "53"
# cohort <- "Both"

cat("run Cox-models with", phe, "in", cohort, "with")
...
ukb.cov         <- fread("/sc-projects/sc-proj-computational-medicine/people/Maik/51_protein_variance_updated/01_phenotype_prep/data/UKB.variables.variance.decomp.imputed.dataset.1.20280128.txt",
                         select=c("f.eid", covs))

## decide whether or not the outcome is sex-specific
if(cohort != "Both"){
  ukb.cov <- ukb.cov[ sex == cohort ]
...
if(length(covs) != 1){
  tmp             <- model.matrix.lm(as.formula(paste0("~ ", paste(covs, collapse = " + "))), ukb.cov, na.action = "na.pass")
  ## add
...
## relevant proteins
ukb.prot                <- fread("/sc-projects/sc-proj-computational-medicine/people/Maik/51_protein_variance_updated/01_phenotype_prep/data/UKB.Olink.var.decomp.20250128.txt")

## get new feature vector (differs for genetic scores)
prots.exp       <- intersect(lab.prot$id, names(ukb.prot)[-1])

## --> relevant phecode <-- ##
## update to new set of phecodes
ukb.phe        <- fread("input/UKB.olink.phecodes.20250516.txt", select = c("f.eid", paste0("bin_", phe), paste0("surv_", phe)))
...
if(sum(ukb.dat[, paste0("bin_", phe), with=F]) >= 200){
  ...
  ## store the results
  fwrite(res.surv, paste("output/res.surv", phe, "txt", sep="."), sep="\t", row.names = F, na = NA)
