...
## --> import protein levels adjusted for top 3 determinants <-- ##
ukb.prot <- fread("input/slice_10UKB.olink.proteins.full.20250516.txt")
...
ukb.dat        <- merge(ukb.dat, ukb.prot, by="f.eid")
...
if(sum(ukb.dat[, paste0("bin_", phe), with=F]) >= 200){
  ...
  ## store the results
  fwrite(res.surv, paste("output/res.surv.adj.top10", phe, "txt", sep="."), sep="\t", row.names = F, na = NA)
